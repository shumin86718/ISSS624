...
In this hands-on exercise 3, we will gain hands-on experience on how to delineate homogeneous region by using geographically referenced multivariate data. There are two major analysis, namely:
...
-   Multivariate data visualisation and analysis
    ...
    -   **coorplot (correlation plot)**, **ggpubr**, and **heatmaply**
...
In order to overcome this problem, we will derive the penetration rate of each ICT variable by using the code chunk below. **(per thousand households, not percentage)**
...
Review the summary statistics of the newly derived penetration rates using the code chunk below.
...
Insert the plot photo by Image function

![](/_site/Hands-on_Ex/Hands-on_Ex3/Hands-on_Ex3_files/figure-html/unnamed-chunk-9-1.png)
...
The code chunks below is used to perform the task. The unique identifier used to join both data objects is *TS_PCODE*. **(column name already being changed to the same, actually no need line 3 code, "by=c("TS_PCODE"="TS_PCODE")")**
...
In this section, you will learn how to use [*corrplot.mixed()*](https://cran.r-project.org/web/packages/corrplot/corrplot.pdf) function of [**corrplot**](https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html) package to visualise and analyse the correlation of the input variables. **(only pick up columns 12 to 17)**
...
We can then plot the tree by using *plot()* of R Graphics as shown in the code chunk below. **(cex scaled the resolution to 60%, so the column name won't overlapped)**
...
**(agnes() function gives the best value; function(x)'s x is same as method = x, measures how homogenizes???, refer to agnes() function in reference)**

**function() is a looping, refer to function() reference file.**
...
cluster 1 looks high intensity in MPHONE_PR and COMPUTER_PR and TV_PR

cluster 2 seems high MPHONE_PR and TV_PR

each cluster can plot as box-plot, able to see the mean and upper and lower quatiles
...
**The output is called *groups*. It is a *list* object, save the clusters dataframe to groups.**
...
The code chunk below uses [*as_Spatial()*](https://r-spatial.github.io/sf/reference/coerce-methods.html) of **sf** package to convert *shan_sf* into a SpatialPolygonDataFrame called *shan_sp*, it is different from shan_sf which without geometric column, polygon list stored separately as a list.
...
Contiguity method, it needs to have neighbours.
...
In order to achieve this, [*nb2listw()*](https://r-spatial.github.io/spdep/reference/nb2listw.html) of **spdep** package is used as shown in the code chunk below, convert it to a weight matrix **shan.w**.
...
The code chunk below is used to plot the newly derived clusters by using SKATER method. **do not do any sorting when do table joining, why?**
...
**D0 black is the attribute space, D1 red is the spatial space, choicealpha() balance both spatial and attribute homogeneity. When is 1, it is the perfect homogenizes, when it is smaller, it reduced the homogenizes.**

seq(0,1,**0.1**) **0.1 is the increment.**

**K is the uppercase K is not the same as small letter k**

**1st plot is the normalized one, 2nd is not the non-normalized, so the normalized D0 lost 20%, and D1 reduce 40% homogeneity.**
