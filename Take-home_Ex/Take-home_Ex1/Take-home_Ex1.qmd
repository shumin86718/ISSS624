...
```{r}
mean(bperm_nonfun$res[1:999])
```

```{r}
var(bperm_nonfun$res[1:999])
```

```{r}
summary(bperm_nonfun$res[1:999])
```
...
```{r}
localMI <- localmoran(nigeria_sf26391$`nonfunctional_ratio`, rswm_q)
head(localMI)
```
...
```{r}
nci2 <- moran.plot(nigeria_sf26391$Z.nonfunctional_ratio, rswm_q,
                   labels=as.character(nigeria_sf26391$shapeName),
...
```{r}
nigeria_sf26391$lag_nonfunctional_points <- lag.listw(rswm_q, nigeria_sf26391$`nonfunctional_ratio`)
DV <- nigeria_sf26391$lag_nonfunctional_points - mean(nigeria_sf26391$lag_nonfunctional_points)     
```
