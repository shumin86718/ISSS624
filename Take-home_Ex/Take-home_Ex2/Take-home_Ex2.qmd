...
If you open *wp_nga* data frame and scroll to the last field now, you will see a new field called *Geometry* has been added as shown below. ![](images/image2.jpg){width="551"}
...
We will use the duplicated function to check all shapeName which have duplcates. (Note: This part of codes cites from the classmate Jordan's Take-Home_Exercies 1).
...
The table below shows the index and the actual name and state of the areas. The table below shows the index and the actual name of the area.
...
Create functional water point count, nonfunctional water point count, unknown water point count, hand pump count, usage capacity count base on less than 1000 and equal or greater than 1000 columns.
...
This is the coordinate system of nga simple feature data frame by using st_crs() of sf package as shown in the code chunk below.
...
Replace NaN with zero value in functional/non functional ratios, percentage of handpump, usageCap less than 1k, usageCap equal to 1k and water point at rural areas.
...
The correlation plot above shows that "pct_handpump","pct_usageCap1k" and "pct_usageCapLess1k" are highly correlated. This suggest that "pct_handpump" should be used in the cluster analysis instead of three variables.
...
cluster_vars <- nga_sf %>%
  ...
  select("shapeName", "pct_functional", "pct_nonfunctional", "pct_handpump", "pct_ruralWP")
head(cluster_vars,10)

cluster_vars.cor = cor(cluster_vars[,2:5])
corrplot.mixed(cluster_vars.cor,
         lower = "ellipse", 
               upper = "number",
               tl.pos = "lt",
...
Notice that the final clustering variables list does not include variable "pct_usageCapLess1k" because it is highly correlated with variable pct_handpump" and "pct_usageCap1k".
...
```{r}
nga_cluster_var <- select(cluster_vars, c(2:5))
...
The code chunk below is used to compute the proximity matrix using *manhattan* method as we are having multiple dia.
...
In this exercise, Gap statistic Method will be used.
...
With reference to the gap statistic graph above, the recommended number of cluster to retain is 1. However, it is not logical to retain only one cluster. By examine the gap statistic graph, the next best 6-cluster gives the largest gap statistic and should be the next best cluster to pick.

## **Silhouette Method**

```{r}
set.seed(1234)
fviz_nbclust(nga_cluster_var, hcut, method = "silhouette", k.max = 10) + theme_minimal() + ggtitle("The Silhouette Plot")
```
...
fviz_nbclust(nga_cluster_var, kmeans, method = "wss", k.max = 10) + theme_minimal() + ggtitle("the Elbow Method")
```
...
With closed examination of the dendragram above, we have decided to retain nine clusters.

[*cutree()*](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/cutree.html) of R Base will be used in the code chunk below to derive a 9-cluster model.
...
From the summary report that we know there is 1 region 86 without any neighbour. To prevent error when we compute the edge cost in next section, we will recommend to remove this region 86 from the dataframe nga_sp.
...
We can plot the neighbours list on nga_sp by using the code chunk below. Since we now can plot the community area boundaries as well, we plot this graph on top of the map. The first plot command gives the boundaries. This is followed by the plot of the neighbor list object, with coordinates applied to the original SpatialPolygonDataFrame (Nga state township boundaries) to extract the centroids of the polygons. These are used as the nodes for the graph representation. We also set the color to blue and specify add=TRUE to plot the network on top of the boundaries.
