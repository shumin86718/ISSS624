---
title: "Take-home_Ex2 Regionalisation with Spatially Constrained Cluster Analysis"
editor: visual
author: HanShumin
date: 11 Dec 2022
---
...
tm_shape(nga_sf) +
  ...
  tm_legend(legend.position = c("right", "bottom"), legend.title.size = 2,legend.text.size = 1.8)+
...
The correlation plot above shows that "pct_handpump","pct_usageCap1k" and "pct_usageCapLess1k" are highly correlated. This suggest that "pct_handpump" and "pct_usageCap1k" should be used in the cluster analysis instead of three variables.
...
cluster_vars <- nga_sf %>%
  ...
  select("shapeName", "functional","nonfunctional", "pct_functional", "pct_nonfunctional", "pct_handpump","pct_usage1k", "pct_ruralWP")
head(cluster_vars,10)

cluster_vars.cor = cor(cluster_vars[,2:8])
corrplot.mixed(cluster_vars.cor,
         lower = "ellipse", 
               upper = "number",
               tl.pos = "lt",
...
```{r}
nga_cluster_var <- select(cluster_vars, c(2:8))
...
The code chunk below is used to compute the proximity matrix using *manhattan* method.

```{r}
proxmat <- dist(nga_cluster_var, method = 'manhattan')
```
...
nga_sf_cluster <- nga_sf_cluster[-c(86),]
